...
describe('template spec', () => {
  ...
  it('OTDR Input by vendor', () => {
    ...
    cy.wait(1000);
    // ✅ Check and click the PICA button if found and enabled
    cy.get('body').then(($body) => {
      if ($body.find('#btnPICABillableOrder').length > 0) {
        cy.get('#btnPICABillableOrder').then(($btn) => {
          if (!$btn.is(':disabled')) {
            cy.wrap($btn)
              .scrollIntoView()
              .should('be.visible')
              .click();
            cy.log('✅ Clicked PICA button');
          } else {
            cy.log('⚠️ PICA button is disabled, skipping click.');
          }
        });
      } else {
        cy.log('⚠️ PICA button not found, skipping.');
      }
    });
    ...
    // ✅ Handle PICA buttons dynamically
    const picaButtons = [
      '#btnPICABillableOrder',
      '#btnPICAPhysicalOrder',
      '#btnPICAImplementedOrder'
    ];

    cy.get('body').then(($body) => {
      let clicked = false;

      for (const btn of picaButtons) {
          ...
          cy.get(btn).then(($btn) => {
            if (!$btn.is(':disabled')) {
              cy.wrap($btn)
                ...
                .should('be.visible')
                .click();
              cy.log(`✅ Clicked PICA button: ${btn}`);
              clicked = true;
            }
          });
          break;
      ...
      if (!clicked) cy.log('⚠️ No PICA button found or all disabled.');
    });

    // ✅ After clicking, handle upload + submit for each popup
    cy.wait(2000);
    cy.get('body').then(($body) => {
      if ($body.find('#flePICABillableDocument').length > 0) {
        cy.get('#flePICABillableDocument').attachFile(PDFFilepath);
        cy.wait(1000);
        cy.get('#btnSubmitPICABillable').click();
        cy.log('📤 Uploaded and submitted Billable PICA.');
      } else if ($body.find('#btnSubmitPICAPhysical').length > 0) {
        cy.get('#flePICABillableDocument').attachFile(PDFFilepath);
        cy.wait(1000);
        cy.get('#btnSubmitPICAPhysical').click();
        cy.log('📤 Uploaded and submitted Physical PICA.');
      } else if ($body.find('#btnSubmitPICAImplemented').length > 0) {
        cy.get('#flePICABillableDocument').attachFile(PDFFilepath);
        cy.wait(1000);
        cy.get('#btnSubmitPICAImplemented').click();
        cy.log('📤 Uploaded and submitted Implemented PICA.');
      } else {
        cy.log('⚠️ No PICA popup upload or submit button found.');
      }
    });
