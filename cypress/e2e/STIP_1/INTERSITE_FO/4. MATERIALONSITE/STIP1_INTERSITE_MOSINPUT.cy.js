...
describe('template spec', () => {
  ...
  it('OTDR Input by vendor', () => {
    ...
    // Check if #slsMobilePIC exists before running the selection process
    cy.get('body').then(($body) => {
      if ($body.find("#slsMobilePIC").length > 0) {
        ...
        cy.get("#PopUpMobilePIC").invoke("show"); // Forces visibility
        cy.get("#btnMobilePICSubmit").should("be.visible").click({ force: true });
        cy.wait(5000);
