...
describe('template spec', () => {
  ...
  beforeEach(() => {
    cy.readFile('cypress/e2e/STIP_1/PKP/NEW_BUILD_MACRO/soDataNewBuild.json').then((values) => {
      cy.log(values);
      sonumb = values.soNumber;
      siteId = values.siteId;
    });

    cy.readFile('cypress/e2e/STIP_1/PKP/NEW_BUILD_MACRO/DataVariable.json').then((values) => {
  ...
  for (let i = 0; i < loopCount; i++) {
    it.only(`passes iteration ${i + 1}`, () => {
      ...
      cy.url().should('include', `${baseUrlTBGSYS}/Dashboard`); // Ensure the page changes or some result occurs
      testResults.push({
        Test: 'Button Clicked',
        Status: 'Pass',
        Timestamp: new Date().toISOString(),
      ...
      cy.get('#tbxNewDocumentName').type('DocName_' + unique + RangerandomValue);
      ...
      cy.get('@siteId').then((siteId) => {
        ...
        cy.get('@soNumber').then((soNumber) => {
          cy.get('@siteId').then((siteId) => {
            const filePath = Cypress.config('fileServerFolder') + '/cypress/e2e/STIP_1/PKP/NEW_BUILD_MACRO/soDataNewBuild.json';
