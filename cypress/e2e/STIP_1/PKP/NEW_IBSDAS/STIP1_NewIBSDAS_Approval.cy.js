...
describe('template spec', () => {
  ...
  let sonumb, siteId, unique, date, userAM, userLeadAM, userLeadPM, userARO, pass, userPMFO, userInputStip, baseUrlVP, baseUrlTBGSYS, login, dashboard, menu1, menu2, menu3, menu4, logout, userPMSitac, userPMCME, userFieldControler, userSitac;
  ...
  beforeEach(() => {
    cy.readFile('cypress/e2e/STIP_1/PKP/NEW_IBSDAS/soDataNewIBS.json').then((values) => {
      cy.log(values);
      sonumb = values.soNumber;
      siteId = values.siteId;
    });
    cy.readFile('cypress/e2e/STIP_1/PKP/NEW_IBSDAS/DataVariable.json').then((values) => {
      cy.log(values);
      ...
      userSitac = values.userSitac;
      userFieldControler = values.userFieldControler;
      pass = values.pass;
      ...
      if (text.includes("Lead PM")) {  // ✅ Flexible condition to match variations
        ...
        cy.get("#btnConfirm").then(($btn) => {
          if ($btn.is(':visible') && !$btn.is(':disabled')) {
            ...
            cy.get('#slsSitacOfficer').select(userSitac, { force: true });
            cy.get('#slsFieldController').select(userFieldControler, { force: true });
