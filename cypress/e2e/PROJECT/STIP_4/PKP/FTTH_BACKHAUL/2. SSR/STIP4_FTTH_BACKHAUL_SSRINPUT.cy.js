...
describe('template spec', () => {
  ...
  beforeEach(() => {
    ...
    cy.readFile('cypress/e2e/STIP_4/PKP/FTTH_BACKHAUL/soDataBackhaul.json').then((values) => {
      cy.log(values);
      sonumb = values.soNumber;
      siteId = values.siteId;
    });

    cy.readFile('cypress/e2e/STIP_4/PKP/FTTH_BACKHAUL/DataVariable.json').then((values) => {
      cy.log(values);
  ...
  it('OTDR Input by vendor', () => {
    ...
    cy.wait(1000);
    cy.get('#tbxSSRLongitudeA').type(long);
    cy.wait(1000);
    ...
    cy.wait(1000);
    cy.get('#tbxSSRLongitudeB').type(long);
    cy.wait(1000);
