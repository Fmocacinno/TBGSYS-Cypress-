...
describe('template spec', () => {
  ...
  let sonumb, siteId, unique, date, userAM, userLeadAM, userLeadPM, userARO, pass, userPMFO, userInputStip, UserRequestSPKProject, Uservendor, baseUrlVP, baseUrlTBGSYS, login, dashboard, menu1, menu2, menu3, menu4, logout, UservendorDespon, userPMSitac;
  ...
  beforeEach(() => {
    ...
    cy.readFile('cypress/e2e/STIP_1/TBG/NEW_BUILD_MACRO/DataVariable.json').then((values) => {
      ...
      UservendorDespon = values.UservendorDespon;
  ...
  it('Material ON Site Approval PM FO', () => {
    ...
    cy.get('tbody tr:first-child td:nth-child(1) .btnSelect')
    ...
    cy.get('#slSubCore').then(($select) => {
      cy.wrap($select).select('28', { force: true })
    })
    cy.wait(3000);
    ...
    cy.wait(5000)
    cy.get('#tbxSearchVendorName').type(UservendorDespon);
    cy.wait(8000)
    cy.get('td[rowspan="1"][colspan="1"]')
      .eq(1)
      .find('.btnSearch')
