...
describe('template spec', () => {
  ...
  it('OTDR Input by vendor', () => {
    ...
    cy.wait(3000);
    cy.get('#tbxCoreLengthSiteGenerate').type(randomValue);
    cy.wait(2000);
    cy.get('#tbxCableModelSiteGenerate').type('Cable Model' + randomString + randomValue);
    cy.wait(2000);
    cy.get('#tbxCableTypeSiteGenerate').type('Cable type' + randomString + randomValue);
    cy.wait(2000);
    cy.get('#tbxTotalHaspleSiteGenerate').type(randomValue);
    cy.wait(2000);
    cy.contains('label', '1550 Nm').click({ force: true });

    cy.get('.col-md-6.col-sm-12 select[name="tblSummaryCoreManagement_length"]')
    ...
    // cy.get('tr[role="row"]').each(($row, index) => {
    //   const rowIndex = index + 1; // IDs use 1-based numbering like tbxLengthOnSite1, tbxSOR1, etc.

    //   // Generate values based on index and your declared variables
    //   const lengthOnSite = randomValue + rowIndex;
    //   const totalSplice = rowIndex * 5;
    //   const lossFiber = (Math.random() * 1).toFixed(2);
    //   const lossSplice = (Math.random() * 1).toFixed(2);
    //   const lossConnector = (Math.random() * 1).toFixed(2);
    //   const totalLoss = (parseFloat(lossFiber) + parseFloat(lossSplice) + parseFloat(lossConnector)).toFixed(2);
    //   const remark = `Automation Remark ${randomString}-${rowIndex}`;

    //   // Fill input fields by ID
    //   cy.get(`#tbxLengthOnSite${rowIndex}`).clear().type(lengthOnSite.toString());
    //   cy.get(`#tbxTotalSpliceOnsite${rowIndex}`).clear().type(totalSplice.toString());
    //   cy.get(`#tbxLossFiber${rowIndex}`).clear().type(lossFiber.toString());
    //   cy.get(`#tbxLossSplice${rowIndex}`).clear().type(lossSplice.toString());
    //   cy.get(`#tbxLossConnector${rowIndex}`).clear().type(lossConnector.toString());
    //   cy.get(`#tbxTotalLoss${rowIndex}`).clear().type(totalLoss.toString());

    //   // Upload files using attachFile
    //   cy.get(`#tbxPhoto${rowIndex}`).attachFile(photoFilePath);
    //   cy.get(`#tbxSOR${rowIndex}`).attachFile(sorFilePath);

    //   // You can log each step if needed
    //   cy.log(`✅ Row ${rowIndex} filled with values.`);
    // });

    cy.get('tr[role="row"]').each(($row, index, $rows) => {
      const rowIndex = index + 1;

      // Build selector for the first field to validate this row is a valid form row
      const selector = `#tbxLengthOnSite${rowIndex}`;

      // Use Cypress.$ (jQuery) to check existence synchronously
      if (Cypress.$(selector).length) {
        ...
        cy.log(`✅ Row ${rowIndex} filled.`);
      } else {
        cy.log(`⛔ Skipping Row ${rowIndex} — Form inputs not found.`);
        // cy.get('#someContainer .nav-tabs a[href="#tabOTDRFarEnd"]').click();
        cy.get('.nav-tabs a[href="#tabOTDRFarEnd"]').first().click();
