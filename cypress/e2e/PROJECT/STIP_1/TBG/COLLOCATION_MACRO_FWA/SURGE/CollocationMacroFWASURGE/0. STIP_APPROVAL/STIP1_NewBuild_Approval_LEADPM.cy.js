...
let sonumb, siteId, unique, date, userAM, userLeadAM, userLeadPM, userARO, pass, userPMFO, userInputStip, userPMCME;
      ...
      if (text.includes("Waiting for Confirmation Lead PM")) {  // ✅ Flexible condition to match variations
        ...
        cy.get("#btnConfirm").then(($btn) => {
          if ($btn.is(':visible') && !$btn.is(':disabled')) {
            ...
            cy.get('#slsFieldController').select(userPMFO, { force: true });
