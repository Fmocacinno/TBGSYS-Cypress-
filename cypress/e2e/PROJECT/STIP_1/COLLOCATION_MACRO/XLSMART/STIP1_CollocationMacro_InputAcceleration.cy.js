  ...
  for (let i = 0; i < loopCount; i++) {
    it.only(`passes iteration ${i + 1}`, () => {
      ...
      cy.get('#slsColocationCustomer').then(($select) => {
        cy.wrap($select).select('XLSMART', { force: true })
          .should('have.value', 'XLSMART') // Menunggu hingga value benar-benar berubah
          .then(() => {
            // Log the test result if selection is successful
            testResults.push({
              Test: 'User memilih Customer di dropdown Colocation Customer',
      ...
      cy.get('#slsColocationMLANumber').then(($select) => {
        cy.wrap($select).select('XLSMART0010-14-F07-390333', { force: true })
      })
      ...
      cy.get('@siteId').then((siteId) => {
        ...
        cy.get('@soNumber').then((soNumber) => {
          cy.get('@siteId').then((siteId) => {
            const filePath = Cypress.config('fileServerFolder') + '/cypress/e2e/STIP_1/TBG/COLLOCATION_MACRO/soDataCOLLOCATION_MACRO.json';
