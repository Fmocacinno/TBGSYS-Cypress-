...
describe('template spec', () => {
  ...
  for (let i = 0; i < loopCount; i++) {
    ...
    it(`THIS FLOW SUBMIT CRQ OPERATOR WITH REQUEST TYPE Additional Core Intersite ${i + 1}`, () => {
      ...
      cy.get('#slTypeNewPullingCableIN1').then(($select) => {
        cy.wrap($select).select('24', { force: true })
      })

      cy.get('#slLastMileCableOwnershipIN1').then(($select) => {
        cy.wrap($select).select('TBG', { force: true })
      })
      cy.get('#slLastMileCableOwnershipIN1').then(($select) => {
        cy.wrap($select).select('TBG', { force: true })
      })
      ...
      cy.get('#txtNewPullingCableIN1').type(randnumber(5), { force: true });

      cy.get('#slImplementationTypeOUT1').then(($select) => {
        cy.wrap($select).select('Aerial', { force: true })
      })

      cy.get('#slTypeNewPullingCableOUT1').then(($select) => {
        cy.wrap($select).select('24', { force: true })
      })

      cy.get('#slLastMileCableOwnershipOUT1').then(($select) => {
        cy.wrap($select).select('TBG', { force: true })
      })
      cy.get('#slLastMileCableOwnershipOUT1').then(($select) => {
        cy.wrap($select).select('TBG', { force: true })
