...
describe('template spec', () => {
  ...
  beforeEach(() => {
    cy.readFile('cypress/e2e/ASSET/TBG/TSEL/MODUL_PV/soDataMaintenance.json').then((values) => {
      cy.log(values);
      sonumb = values.soNumber;
      siteId = values.siteId;
    });
    cy.readFile('cypress/e2e/ASSET/TBG/TSEL/MODUL_PV/DataVariable.json').then((values) => {
      cy.log(values);
